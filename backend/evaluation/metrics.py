...
def _contains_any(text: str, snippets: list[str]) -> bool:
    ...
    return any(" ".join(snippet.lower().split()) in normalized for snippet in snippets)
...
def summarize(question_results: list[dict], k: int) -> RetrievalReport:
    """
    ...
    return RetrievalReport(
        ...
        k=k,
        hit_rate=sum(r["hit_at_k"] for r in question_results) / len(question_results),
        recall=sum(r["recall_at_k"] for r in question_results) / len(question_results),
    )
