...
class InMemoryIndex:
    ...
    def query(self, vector, top_k, include_metadata=False, filter=None):
        ...
        scored = [
            {"score": self._cosine(vector, v["values"]), "metadata": dict(v["metadata"])}
    ...
    def delete(self, filter=None):
        name = (filter or {}).get("pdf_name")
        self.vectors = [
            v for v in self.vectors if v["metadata"]["pdf_name"] != name
        ]
...
class TestEvaluator:
    ...
    def synthetic_setup():
        ...
        token_map = {"gold": 0, "alpha": 1, "beta": 2, "marker": 3, "both": 4, "one": 5, "none": 6}
        seeds = {
            "both-gold": ["both marker chunk with gold alpha", "both marker chunk with gold beta"],
                ...
                index.vectors.append(
                    {
                        ...
                        "metadata": {"content": content, "pdf_name": "synthetic.pdf", "chunk_index": i},
                    }
                )
        for i in range(8):
            ...
            index.vectors.append(
                {
                    "id": f"noise-{i}",
                    "values": one_hot_embed([content], token_map)[0],
                    "metadata": {"content": content, "pdf_name": "synthetic.pdf", "chunk_index": 10 + i},
                }
            )
...
class TestLiveGate:
    ...
    def test_cli_json_output_shape(self, fixture, indexed_index, monkeypatch, capsys):
        monkeypatch.setattr(cli, "run", lambda live, judge, k: {
